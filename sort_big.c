...
void	ft_rotate_stack_top(t_node **stack, t_node *stack_top, char *stack_name)
{
	...
	while (*stack != stack_top)//iterate until the desired number is in the top of stack
	{
		...
		{
			if (stack_top->arriba_abajo == true)//check for above/below avg and move it accordingly
				ra(stack);
			else
				rra(stack);
		}
...
void	ft_push_from_b_to_a(t_node **stack_a, t_node **stack_b)
{
	...
	{
		if (node->arriba_abajo == true && node->target_node->arriba_abajo == true)//if both are above average rotate to make it go up and reach the top
			rr(stack_a, stack_b);
		else if (node->arriba_abajo == false && node->target_node->arriba_abajo == false)//if both are below average reverse rotate to make it go down until reach the top
			rrr(stack_a, stack_b);
		stack_a = &(*stack_a)->next;//iterate
		stack_b = &(*stack_b)->next;//iterate
	}
	printf("calling ft_above_avg\n");
	ft_arriba_abajo(*stack_a);//update the above avg flag
	ft_arriba_abajo(*stack_b);//update the above avg flag
...
void	ft_sort_stack_big(t_node **stack_a, t_node **stack_b)
{
	...
	val = minimo->value;//minimum value
